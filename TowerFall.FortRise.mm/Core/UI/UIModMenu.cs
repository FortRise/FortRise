...
public class UIModMenu : CustomMenuState
{
    ...
    public override void Create()
    {
        ...
        foreach (var mod in FortRise.RiseCore.InternalFortModules) 
        {
            ...
            string setupName;
            if (!string.IsNullOrEmpty(mod.Meta.DisplayName))
            {
                setupName = mod.Meta.DisplayName + " v" + version.ToUpperInvariant();
            }
            else 
            {
                setupName = mod.Meta.Name + " v" + version.ToUpperInvariant();
            }
