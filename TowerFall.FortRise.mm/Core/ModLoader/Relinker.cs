...
{
    internal readonly static Dictionary<string, ModuleDefinition> StaticRelinkModuleCache = new() 
    {
        { 
            "MonoMod", ModuleDefinition.ReadModule(
                typeof(MonoModder).Assembly.Location, 
                new ReaderParameters(ReadingMode.Immediate)) 
        },
        { 
            "TowerFall", ModuleDefinition.ReadModule(
                typeof(TFGame).Assembly.Location, 
                new ReaderParameters(ReadingMode.Immediate)) 
        }
    };
    ...
    public static Dictionary<string, ModuleDefinition> SharedRelinkModuleMap
    {
        get 
        {
            ...
            sharedRelinkModuleMap = [];
            string[] entries = Directory.GetFiles(RiseCore.GameRootPath);
            for (int i = 0; i < entries.Length; i++) 
            {
                var path = entries[i];
                var name = Path.GetFileName(path);
                var nn = name[..Math.Max(0, name.Length - 4)];
                ...
                if (name.EndsWith(".mm.dll")) 
                {
                    if (name.StartsWith("TowerFall.")) 
                    {
                        ...
                        RiseCore.logger.LogInformation("[Relinker] Relinking {name}.", name);
                    }
                    else 
                    {
                        RiseCore.logger.LogWarning("[Relinker] Found unknown {name}.", name);
                        ...
                        string nameRelinkedNeutral = name[..dot];
                        string nameRelinked = nameRelinkedNeutral + ".dll";
                        string pathRelinked = Path.Combine(
                                Path.GetDirectoryName(path), nameRelinked);
                        if (!File.Exists(pathRelinked))
                        {
                            continue;
                        }

                        if (!StaticRelinkModuleCache.TryGetValue(
                                nameRelinkedNeutral, out ModuleDefinition relinked)) 
                        {
                            relinked = ModuleDefinition.ReadModule(
                                pathRelinked, 
                                new ReaderParameters(ReadingMode.Immediate)
                            );
                        ...
                        RiseCore.logger.LogTrace(
                            "[Relinker] Remapped to {relinkedAsm}.", nameRelinked
                        );
    ...
    public static Dictionary<string, object> SharedRelinkMap 
    {
        get 
        {
            ...
            for (int i = 0; i < asmRefs.Length; i++) 
            {
                ...
                if (!asmRef.FullName.Contains(
                        "fna", StringComparison.InvariantCultureIgnoreCase) &&
                    !asmRef.FullName.Contains(
                        "xna", StringComparison.InvariantCultureIgnoreCase) &&
                    !asmRef.FullName.Contains(
                        "monogame", StringComparison.InvariantCultureIgnoreCase))
                {
                    continue;
                }
                ...
                SharedRelinkModuleMap[asmRef.FullName] 
                    = SharedRelinkModuleMap[asmRef.Name] 
                    = module;
                ...
                for (int k = 0; k < types.Length; k++) 
                {
                    ...
                    var typeDef = module.GetType(type.FullName) 
                        ?? module.GetType(type.FullName.Replace('+', '/'));
                    if (typeDef == null)
                        continue;
        ...
        ModuleMetadata meta, ReadOnlySpan<char> name, Stream stream) 
    {
        ...
        var cachedChecksumPath = string.Concat(
                cachedPath.AsSpan(0, cachedPath.Length - 4), ".sum");
        ...
        try 
        {
            ...
            if (modder.ReaderParameters.SymbolReaderProvider != null && 
                modder.ReaderParameters.SymbolReaderProvider is RelinkerSymbolReaderProvider p) 
            {
                p.Format = DebugSymbolFormat.PDB;
            }
            ...
            if (modder.ReaderParameters.SymbolReaderProvider != null && 
                modder.ReaderParameters.SymbolReaderProvider is RelinkerSymbolReaderProvider p1) 
            {
                p1.Format = DebugSymbolFormat.Auto;
            }
            ...
            if (runtimeRulesModule == null) 
            {
                string rulesPath = Path.Combine(
                    ...
                    Path.GetFileNameWithoutExtension(
                        typeof(TFGame).Assembly.Location) + ".FortRise.mm.dll");
                ...
                if (!File.Exists(rulesPath))
                {
                    throw new InvalidOperationException(
                        "Couldn't find runtime rules TowerFall.FortRise.mm.dll");
                }

                if (File.Exists(rulesPath)) 
                {
                    runtimeRulesModule = ModuleDefinition.ReadModule(
                        rulesPath, new ReaderParameters(ReadingMode.Immediate));
                }
            }
            ...
            ISymbolWriterProvider symbolWriterProvider = 
                modder.WriterParameters.SymbolWriterProvider;
            ...
            catch 
            {
                ...
                catch when (!temporaryASM) 
                {
                    RiseCore.logger.LogError(
                        "[Relinker] {cachedPath} is currently in used.", 
                        cachedPath);
                    temporaryASM = true;

                    long stamp2 = DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond;
                    string filename = $"{Path.GetFileNameWithoutExtension(dirPath)}.{stamp2}";

                    cachedPath = Path.Combine(Path.GetTempPath(), 
                        $"FortRise.{asmName}.{filename}.dll");

                    RiseCore.logger.LogInformation(
                        "[Relinker] Moving to {cachedPath}", 
                        cachedPath);
        ...
        {
            RiseCore.logger.LogError("[Relinker] Failed Relinking {meta} - {asm}.", 
                meta, 
                new string(asmName));
            
            RiseCore.logger.LogError("[Relinker] Exception: {exception}", e);
        }
        finally 
        {
            modder.ReaderParameters.SymbolStream?.Dispose();
        ...
        {
            RiseCore.logger.LogError(
                "[Relinker] Failed Loading {meta} - {asm}", 
                meta, 
                new string(asmName));
    ...
    public static Assembly FakeRelink(ModuleMetadata meta, ReadOnlySpan<char> name, Stream stream)
    {
        ...
        {
            RiseCore.logger.LogInformation(
                "[Fake Relinker] Loading cached assembly for {meta} - {asm}", 
                meta, 
                new string(asmName)
            );
...
internal sealed class RelinkerSymbolReaderProvider : ISymbolReaderProvider 
{
    ...
    public ISymbolReader GetSymbolReader(ModuleDefinition module, Stream symbolStream) 
    {
        ...
        {
            DebugSymbolFormat.PDB when IsPortablePdb(symbolStream) => 
            ...
            DebugSymbolFormat.MDB => new MdbReaderProvider().GetSymbolReader(module, symbolStream),
            _ => null
        };
    }
