...
public class ActorLeftButton : EditorUI
{
    ...
    public ActorLeftButton(Vector2 position, patch_EditorScene scene) 
        : base(position, 40, 40, -20, -20)
    {
        Tag(GameTags.EditorActorPalette);
        if (scene.OnActorLayer <= 0)
        {
            subA = TFGame.EditorAtlas["levels/leftNone"];
        }
        else 
        {
            subA = TFGame.EditorAtlas["levels/left"];
        }
    ...
    public bool CanAct
    {
        ...
        {
            return !(
            (Editor as patch_EditorScene)?.OnActorLayer <= 0);
        }
    }
}
