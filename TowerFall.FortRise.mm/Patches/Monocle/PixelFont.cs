...
public class PixelFontCharacter
{
    ...
    public PixelFontCharacter(int character, Texture texture, XmlElement xml)
    {
        Character = character;
        Texture = new Subtexture(texture, xml.AttrInt("x"), xml.AttrInt("y"), xml.AttrInt("width"), xml.AttrInt("height"));
...
public class PixelFontSize
{
    ...
    public void Draw(char character, Vector2 position, Vector2 justify, Vector2 scale, Color color)
    {
        ...
        if (Characters.TryGetValue(character, out c))
        {
            ...
            Monocle.Draw.Texture(c.Texture, Calc.Floor(pos), color, scale.X);
        }
    }
        ...
        for (int i = 0; i < text.Length; i++)
        {
            ...
            if (Characters.TryGetValue(text[i], out c))
            {
                ...
                if (stroke > 0 && !Outline)
                {
                    if (edgeDepth > 0)
                    {
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(0, -stroke), strokeColor, Vector2.Zero, scale);
                        for (var j = -stroke; j < edgeDepth + stroke; j += stroke)
                        {
                            Monocle.Draw.Texture(c.Texture, pos + new Vector2(-stroke, j), strokeColor, Vector2.Zero, scale);
                            Monocle.Draw.Texture(c.Texture, pos + new Vector2(stroke, j), strokeColor, Vector2.Zero, scale);
                        }
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(-stroke, edgeDepth + stroke), strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(0, edgeDepth + stroke), strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(stroke, edgeDepth + stroke), strokeColor, Vector2.Zero, scale);
                    }
                    else
                    {
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(-1, -1) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(0, -1) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(1, -1) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(-1, 0) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(1, 0) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(-1, 1) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(0, 1) * stroke, strokeColor, Vector2.Zero, scale);
                        Monocle.Draw.Texture(c.Texture, pos + new Vector2(1, 1) * stroke, strokeColor, Vector2.Zero, scale);
                ...
                // draw edge
                if (edgeDepth > 0)
                    Monocle.Draw.Texture(c.Texture, pos + Vector2.UnitY * edgeDepth, edgeColor, scale.X);

                // draw normal character
                Monocle.Draw.Texture(c.Texture, pos, color, scale.X);
...
public class PixelFont
{
    ...
    public PixelFontSize AddFontSize(string path, XmlElement data, Atlas atlas = null, bool outline = false)
    {
        ...
        foreach (XmlElement page in pages)
        {
            ...
            if (atlas != null && atlas.Contains(atlasPath))
            {
                Textures.Add(atlas[atlasPath].Texture);
            }
            else
            {
                using var fs = File.OpenRead(path);
                var texture = Texture2D.FromStream(Engine.Instance.GraphicsDevice, fs);
                var subTexture = new Texture(texture);
